...
static float SectorFloor(int s)   { return s != -1 ? sectors[s].floor_z  : 0.0f; }
static float SectorCeiling(int s) { return s != -1 ? sectors[s].ceilingz : 1000.0f; }
...
void PlayerSpawn(Vector2 map_pos, float yaw) {
    ...
    g_pl.pos = SectorToWorld(map_pos, SectorFloor(g_pl.sector));
    g_pl.yaw = yaw;
    g_pl.on_ground = true;
    FLOG_INFO(LOGCAT_GAME, "player spawn at (%.1f, %.1f) sector %d floor %.1f",
              map_pos.x, map_pos.y, g_pl.sector, SectorFloor(g_pl.sector));
}
...
static bool WallBlocks(int w, float feet) {
    int ns = walls[w].next_sector;
    if (ns == NO_LINK) return true;                                  // solid
    if (sectors[ns].floor_z > feet + PL_STEP) return true;           // ledge too high
    float floor_after = fmaxf(feet, sectors[ns].floor_z);
    if (sectors[ns].ceilingz - floor_after < PL_HEIGHT) return true; // can't fit
    return false;
}
...
static void CollideAndSlide(void) {
    for (int pass = 0; pass < 3; pass++) {
        ...
        for (int w = 0; w < wall_counter; w++) {
            if (!WallBlocks(w, g_pl.pos.y)) continue;
...
void PlayerUpdate(float dt) {
    ...
    float floor = SectorFloor(g_pl.sector);
    float ceil  = SectorCeiling(g_pl.sector);

    if (g_pl.on_ground) {
        if (floor >= g_pl.pos.y - PL_STEP && floor <= g_pl.pos.y + PL_STEP) {
            g_pl.pos.y = floor;          // stick to stairs, up and down
            g_pl.vel.y = 0.0f;
...
void PlayerApplyCamera(Camera *camera) {
    Vector3 eye = { g_pl.pos.x, g_pl.pos.y + PL_EYE, g_pl.pos.z };
    Vector3 dir = { cosf(g_pl.yaw) * cosf(g_pl.pitch), sinf(g_pl.pitch),
                    sinf(g_pl.yaw) * cosf(g_pl.pitch) };
    camera->position = eye;
